...
if(!$export)
print_header('EMBA Registration Report', 'EMBA Registration Report', $navigation, '', '', true, '', user_login_string($SITE).$langmenu);

if($courses = get_courses($categoryid, '', 'c.id, c.fullname, c.startdate, c.idnumber, c.shortname') or $export){
	$mform = new mod_custom_reports_view_attendance_report_form('Emba_reg_report.php', array('courses'=>$courses, 'categoryid'=>$categoryid));
	if($fromform = $mform->get_data() or $export){
		...
		if($export){
			...
			echo '<div style="text-align: center; font-weight: bold;">EMBA Students Registration Report </div>';
...
function ExportToExcel($data) {
	...
	$myxls->write(1, 0, "EMBA Registration Report",$header1);
